...
const AdminComponent = () => {
    ...
    const [filteredAdmins, setFilteredAdmins] = useState([]);  // New state for filtered results
    ...
    const handleSearchSubmit = (event) => {
        event.preventDefault();
        if (searchId) {
            const foundAdmin = admins.find(admin => admin.admin_id.toString() === searchId);
            setFilteredAdmins(foundAdmin ? [foundAdmin] : []);
        } else {
            setFilteredAdmins(admins);  // Reset filteredAdmins to show all admins
        }
    };
